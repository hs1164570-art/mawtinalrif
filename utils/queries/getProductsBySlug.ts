...
// ✅ أخف select ممكن - بس اللي بيتعرض في الكارد + القيك فيو
const productSelect = {
  id: true,
  name: true,
  price: true,
...
export async function getProductsData(
  ...
  // ─── 2. لو عنده أولاد → دور في منتجات الأولاد، لو ورقة → دور فيه ───
  const categoryIds =
  ...
  const [products, total, priceAgg] = await Promise.all([
    ...
    // السعر المين والماكس بتاع الكتيجوري كلها (مش بس الفلتر الحالي)
