    ...
    >
      {payload?.map((entry: any, i: number) => (
        <li key={i} className="flex items-center gap-1">
          ...
          />
          <span className="text-[10px] text-[#6B4C3B]">{entry.value}</span>
        </li>
      ))}
    </ul>
  );
      ...
      : <>
          ...
          <div className="flex justify-center -mt-10 pb-4">
            <div className="text-center">
              <p className="text-xl font-bold text-[#3D2B1F] tabular-nums">
                {total.toLocaleString("en-US")}
              </p>
              <p className="text-[10px] text-[#A89585]">إجمالي الطلبات</p>
            </div>
          </div>
        </>
      }
