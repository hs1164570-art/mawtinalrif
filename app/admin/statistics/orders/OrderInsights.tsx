...
import { ClipboardList, CheckCircle2, AlertTriangle, Info, TrendingDown } from 'lucide-react';
...
function buildInsights(data: OrdersData, period: string): InsightItem[] {
  ...
  if (total === 0) {
    items.push({ type: 'info', icon: '📦', message: 'لا توجد طلبات في هذه الفترة. جرّب توسيع نطاق الفترة الزمنية.' });
    return items;
  }
...
const STYLES: Record<InsightItem['type'], { bg: string; border: string; icon: React.ReactNode }> = {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-sm font-semibold text-[#3D2B1F]">تحليلات كفاءة التشغيل</h2>
