...
export default function Sub6Cinematic({
  ...
  return (
    <article
      className="relative bg-[var(--bg)] text-[var(--text-1)] py-10 md:py-24 overflow-hidden border-t border-neutral-100"
    ...
    >
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 flex flex-col gap-5 md:gap-12">
        ...
        <div className="hidden lg:flex flex-col md:flex-row justify-between items-baseline border-b border-neutral-200 pb-4 rtl text-right">
          <div>
            ...
            <h2
              ...
              className="text-[var(--text-1)] text-2xl md:text-4.5xl font-light tracking-tight"
            >
              {subName}
            </h2>
          </div>
          ...
          <Link
            ...
            className="group flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-neutral-500 hover:text-[var(--text-1)] transition-colors"
          >
        ...
        <Link
          ...
          className="group block relative w-full aspect-[16/8] md:aspect-[21/9] overflow-hidden bg-[var(--bg-deep)] shadow-[0_10px_40px_rgba(0,0,0,0.01)] rounded-xl md:rounded-none"
        >
          <Image
            src={cinema.image}
            alt={cinema.name}
          ...
          <div className="absolute bottom-3 right-4 left-4 md:left-auto md:bottom-6 md:right-8 text-right rtl flex items-end justify-between md:justify-start md:flex-col md:gap-1 text-[var(--text-inv)]">
            {/* في الكمبيوتر فقط: الإصدار الرئيسي واسم المنتج بلون أبيض ملكي نقّي */}
            <div className="hidden md:flex flex-col gap-0.5">
              <span className="text-[9px] font-bold text-[var(--text-inv)]/70 tracking-wider uppercase block">
                الإصدار الرئيسي
              </span>
              <h3 className="text-sm font-light text-[var(--text-inv)] line-clamp-1 mb-1">
                {cinema.name}
              </h3>
            </div>
            ...
            <div className="flex items-center gap-1.5 w-full md:w-auto justify-end">
              <span className="text-xs md:text-sm font-bold text-[var(--text-inv)] tracking-wide">
                {cinema.price.toLocaleString("en-US")}ر.س
              </span>
              {/* سهم طائر ناعم في الموبايل للتوجيه البصري */}
              <span className="md:hidden text-[var(--text-inv)]/80">
                <ArrowUpLeft size={13} />
              </span>
            </div>
          </div>
            ...
            return (
              ...
              >
                ...
                <div className="pt-2.5 text-right rtl flex flex-col space-y-0.5 px-1">
                  ...
                  <h4 className="text-[var(--text-1)] text-[11px] sm:text-xs md:text-sm font-medium line-clamp-1 group-hover:text-[var(--gold)] transition-colors">
                    {p.name}
                  </h4>
                  <span className="text-[11px] sm:text-xs font-bold text-[var(--text-2)]">
