// ─────────────────────────────────────────────────────────────────────────────
// app/blog/category/[slug]/page.tsx — صفحة التصنيف (Category) — مدونة موطن الريف
// مؤسسة موطن الريف للتجارة — الرياض
//
// السيو inline بالكامل — مطابق تماماً لباقي ملفات المدونة (صفر ملفات منفصلة)
// ✅ Long-tail title     → "مقالات [التصنيف] في الرياض" — نية بحث طبيعية وقوية
// ✅ CollectionPage + ItemList(BlogPosting) → أقوى من ItemList بسيط
// ✅ generateStaticParams → pre-render كل التصنيفات وقت البناء
// ✅ تصنيف غير موجود → 404 حقيقي + robots noindex (مش صفحة فاضية مفهرسة)
...
export async function generateMetadata({
  ...
  // ✅ تصنيف غير موجود → noindex بدلاً من صفحة فارغة مفهرسة (تضر crawl budget)
  if (!category) {
    return {
      title: "التصنيف غير موجود" + TITLE_SUFFIX,
      robots: { index: false, follow: false },
    };
  }

  const url = `${BASE_URL}${BLOG_BASE_PATH}/category/${slug}`;

  // ✅ Title: نية بحث long-tail طبيعية — "مقالات [التصنيف] في الرياض"
  const title = `مقالات ${category.name} في الرياض | ${BLOG_TITLE}`;
  const description =
    category.description ??
    `كل مقالات قسم ${category.name} في مدونة موطن الريف — نصائح وأفكار حصرية ` +
      `حول ${category.name} للأثاث والديكور المنزلي في الرياض، المملكة العربية السعودية.`;
  ...
  return {
    ...
    // ✅ Canonical ذاتي — صفحة التصنيف لنفسها
    alternates: {
      canonical: url,
      languages: { "ar-SA": url },
    },
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `مقالات ${category.name} — ${BLOG_TITLE}`,
        },
      ],
    },
...
export default async function CategoryPage({ params }: PageProps) {
  ...
  const result = await getPostsByCategory(slug, 1);
  ...
  // ── BreadcrumbList JSON-LD — الرئيسية ← المدونة ← التصنيف ──────────────────
